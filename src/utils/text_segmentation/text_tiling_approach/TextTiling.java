package utils.text_segmentation.text_tiling_approach;

/*  Please see the license information in the header below. */

import utils.text_segmentation.stopwords.BaseStopWords;
import utils.text_segmentation.stopwords.StopWords;
import utils.text_segmentation.struct.RawText;
import utils.text_segmentation.utils.ListFactory;
import utils.text_segmentation.utils.MapFactory;
...
//    }
    ...
    public void similarityDetermination()
    {
        ...
        for ( int i = w; i-- > 0; )
        {
            blockAdd( text.get( i )  , left );
        }

        for ( int i = w * 2; i-- > w; )
        {
            blockAdd( text.get( i ) , right );
        }
